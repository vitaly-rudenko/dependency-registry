...
class DependencyRegistry {
    ...
    registerFactory(...args) {
        ...
        if (args.length === 2) {
            ...
            this._register(
                dependencyName,
                new FactoryWithDependencies(this, factoryMethod),
            );
        }
    }
    ...
    import(registry) {
        ...
        for (const [dependencyName, dependency] of registry._dependencies.entries()) {
            if (dependency instanceof FactoryWithDependencies) {
...
class FactoryWithDependencies {
    ...
    clone(dependencyRegistry) {
        return new FactoryWithDependencies(dependencyRegistry, this._factoryMethod);
    }
